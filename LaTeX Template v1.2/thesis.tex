...
\begin{enumerate}
  ...
  \item 推荐使用Windows下的CTeX环境。CTeX是一个编辑器（WinEdt）加编译器（MiKTeX）的整合包，安装完成之后请务必升级MiKTeX（WinEdt或开始菜单均有升级入口）。Linux或Mac用户可以选择TeX Live （编译器，包含一个简单的编辑器）加TeXStudio或texmaker（编辑器）。
\end{enumerate}
